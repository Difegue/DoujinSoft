...
window.midi_writer = require('midi-writer-js');

//

window.build_file = (mio_data) => {
	
	let MidiWriter = window.midi_writer
	...
	// TODO: To int
	let instrument_codes = ["0","18","6","22","73","56","65","75","24","29","106","33","40","13","11","47","72","78","17","38","77","59","126","124","60","61","62","123","66","125","68","122","53","54","52","49","67","121","119","48","83","84","85","86","87","88","89","90"];
	
	 /*let instrument_lengths = [6f,10f,4f,9f,8f,2f,4f,8f,
	...
	let track_length = 32;
	for (let track_index = 0; track_index < 4; track_index++) {
		var track = new MidiWriter.Track();

		track.setTempo(120);
		
		let song_offset = 0xB961 + track_index * track_length;
		
		let instrument_used = mio_data[0xBA6B + track_index];
		let volume = mio_data[0xBA61 + track_index] * 20;
		console.debug({volume});

		console.debug(instrument_used, 'instrument', instrument_codes[instrument_used]);
		track.addEvent(new MidiWriter.ProgramChangeEvent({instrument: parseInt(instrument_codes[instrument_used])}));

		let notes_used = notes;
		if ([11, 43, 47].includes(instrument_used)) {
			console.debug('lower notes');
			notes_used = lower_notes;
		}

		let note_duration = '16';
		let wait = [];
		console.debug({track_length});
		for (let i = 0; i < track_length; i++) {
			let note = mio_data[song_offset + i];
			if (note === 255) {
			} else {
				let duration = 'T' + (instrument_lengths[instrument_used] * 8);
				console.debug(i, notes_used[note], duration);
				track.addEvent(new MidiWriter.NoteEvent({pitch: notes_used[note],
							duration,
							startTick: 32*i,
							velocity: volume}));
			}
